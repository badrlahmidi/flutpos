...
| **Refonte Statistiques & Reporting** | 🟢 Fait | Plan (`reporting_refonte_plan.md`), 4 rapports Drift, filtres utilisateur/catégorie/session, onglets dynamiques multi-rapports, exports CSV/PDF, dashboard KPI intégré, aperçu A4 avec zoom. | Tests unitaires rapports, en-tête fiscal établissement sur PDF. |
...
| **Utilisateurs & Sécurité (RBAC)** | 🟢 Fait | Migration v8, repositories audités, `SecurityGuard`/`SecurityGate` (pay-in/out, tiroir, backoffice), page admin 2 onglets, tests unitaires. Plan : `users_security_plan.md`. | Garde POS restante (réimpression ticket), prénom/nom utilisateur. |
